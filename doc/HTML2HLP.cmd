...
PARSE VALUE REVERSE( ARG(1) ) WITH .'.'outfile.4
outfile.4 = REVERSE( outfile.4 )
outfile.1 = outfile.4||'_split.html'
outfile.2 = outfile.4||'_style.html'
outfile.3 = outfile.4||'.ipf'
outfile.4 = outfile.4||'.hlp'
...
GetLinkType: PROCEDURE EXPOSE link.
    ...
    ELSE DO
        ...
        ELSE IF link.scheme.clean = link.scheme THEN
...
chk_lnk: PROCEDURE EXPOSE __meter. input. output. i newln
    ...
    DO /* fn */
        output.i = ":link reftype=fn refid='"||link.target.encoded||"'."
    END
    ...
    DO j = j TO input.0
        ...
        PARSE VALUE tag WITH tag'>'.
        IF LEFT( input.j, 1 ) = '<' & LEFT( tag, 1 ) <> '/' THEN DO
            tag_id = parse_xml_tags( input.i, tag, ' id', 5 )
            IF LENGTH( tag_id ) > 0 & tag_id = link_target THEN
            DO
                output.j = ':fn.'
                level = input.j.lvl
                DO k = j + 1 TO input.0
